...
import { FaChevronRight, FaHome, FaPen, FaEnvelope, FaUserFriends, FaBell, FaUsers, FaBook, FaGamepad, FaCog, FaSignOutAlt } from "react-icons/fa"; // Íconos para el menú comprimido
import FRONTIS from "../../assets/FRONTIS.png";

const SideBar = () => {
  const [openMenu, setOpenMenu] = useState(null); // Estado para manejar submenús
  const [open, setOpen] = useState(true); // Estado para el sidebar completo (hamburguesa)
  ...
  const toggleCompact = () => {
    ...
    if (!isCompact) setOpen(false); // Cierra el sidebar completo al activar el modo comprimido
  };
  ...
  return (
    <>
      ...
      {!isCompact && (
        ...
        >
          <div className="h-full bg-gradient-to-b from-[#082F47] via-[#082F47]/100 to-red-700 text-white p-4 overflow-y-auto">
      ...
      {/* Menú Comprimido (a la derecha) */}
      {isCompact && (
        <aside
          className="fixed top-0 right-0 z-40 h-screen w-12 bg-gradient-to-b from-[#082F47] to-red-700 text-white p-2 overflow-y-auto transition-transform duration-300 ease-in-out"
        >
          <div className="flex flex-col items-center space-y-4">
            ...
            <Link to="/" className="text-white hover:text-gray-300"><FaHome size={20} /></Link>
            <Link to="/post" className="text-white hover:text-gray-300"><FaPen size={20} /></Link>
            <Link to="/messages" className="text-white hover:text-gray-300"><FaEnvelope size={20} /></Link>
            <Link to="/friends" className="text-white hover:text-gray-300"><FaUserFriends size={20} /></Link>
            <Link to="/notifications" className="text-white hover:text-gray-300"><FaBell size={20} /></Link>
            <Link to="/group" className="text-white hover:text-gray-300"><FaUsers size={20} /></Link>
            <Link to="/pages" className="text-white hover:text-gray-300"><FaBook size={20} /></Link>
            <Link to="/games" className="text-white hover:text-gray-300"><FaGamepad size={20} /></Link>
            <Link to="/settings" className="text-white hover:text-gray-300"><FaCog size={20} /></Link>
            <Link to="/logout" className="text-white hover:text-gray-300"><FaSignOutAlt size={20} /></Link>
          </div>
        </aside>
      )}

      {/* Botón para togglear modo comprimido (visible en pantallas medianas) */}
      <button
      ...
      >
        {isCompact ? <Menu size={24} /> : <span>F</span>}
      </button>

      {/* Overlay para cerrar el sidebar en móvil (modo completo) */}
      {!isCompact && open && (
        <div

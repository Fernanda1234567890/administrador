...
const AdministrativoVer = () => {
  ...
  const navigate = useNavigate();
  const { getData, updateData, bajaData, restoreData } = administrativosData();
  const { getData: getPerson } = personasData();
  ...
  const handleEliminar = async (id) => {
    if (!window.confirm("¿Seguro que quieres dar de baja este administrativo?")) return;
    await bajaData(id);
    fetchAdministrativos();
  };
  ...
  return (
    <div className="p-6 lg:p-12 min-h-screen dark:bg-white">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-2xl font-bold text-gray-800">Administrativos</h2>
        <button
          onClick={() => navigate("/administrativos/registrar")}
          className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg"
        >
            ...
            <tbody>
              {administrativos.map((adm) => (
                <tr key={adm.id} className="border-b hover:bg-gray-100 transition">
                  <td className="p-3">{adm.id}</td>
                  <td className="p-3">{getNombrePersona(adm.id_persona)}</td>
                  <td className="p-3">{adm.estado ? "Activo" : "Inactivo"}</td>
                  <td className="p-3 flex gap-2">
                    <button
                      onClick={() => handleActualizar(adm)}
                      className="bg-yellow-500 text-white px-2 py-1 rounded hover:bg-yellow-600"

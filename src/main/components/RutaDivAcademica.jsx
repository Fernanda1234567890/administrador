...
const RutaDivAcademica = () => {
  ...
  useEffect(() => {
    const fetchCarreras = async () => {
      try {
        const res = await fetch("http://localhost:3000/api/carreras") // Ajusta tu endpoint
  ...
  // Función para obtener director según el nombre de la carrera
  const normalize = (str) =>
    str?.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "")

  const getDirector = (title) => {
    if (!carreras.length) return "Cargando..."
    const carrera = carreras.find(
      (c) =>
        normalize(title).includes(normalize(c.nombre)) ||
        normalize(c.nombre).includes(normalize(title))
    )
    return carrera ? `${carrera.director.nombres} ${carrera.director.apellidos}` : "Director no asignado"
  }
  ...
  const allItems = [
    {
      ...
      alt: "Carrera Ingenieria de Sistemas",
      title: "INGENIERÍA DE SISTEMAS",
      desc: getDirector("INGENIERÍA DE SISTEMAS"),
      desc2: "Director de Carrera",
    },
    {
      src: sisimg,
      alt: "Carrera Sistemas Tupiza",
      title: "INGENIERÍA DE SISTEMAS TUPIZA",
      desc: getDirector("INGENIERÍA DE SISTEMAS TUPIZA"),
      desc2: "Director de Carrera",
    },
    {
      src: diseñoimg,
      alt: "Diseño Programación",
      title: "PROG. ING. EN DISEÑO EN PROGRAMACIÓN DIGITAL",
      desc: getDirector("PROG. ING. EN DISEÑO EN PROGRAMACIÓN DIGITAL"),
      desc2: "Director de Carrera",
    },
    {
      src: odontoimg,
      alt: "Odontología",
      title: "CARRERA ODONTOLOGÍA",
      desc: getDirector("CARRERA ODONTOLOGÍA"),
      desc2: "Director de Carrera",
    },
    {
      src: facimg,

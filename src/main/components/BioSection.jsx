// import React from 'react';
// import { useNavigate } from 'react-router-dom';

// import rectorImg from '../assets/rector-image.avif';
// import vicerrectorImg from '../assets/vicerrector-image.avif';

// const BioSection = () => {
//   const navigate = useNavigate();

//   const autoridades = [
//     {
//       image: rectorImg,
//       name: 'Ph.D. Pedro Guido Lopez Cortés',
//       title: 'Rector - Universidad Autónoma Tomás Frías',
//       onClick: () => navigate('/rutas-rectorado'),
//     },
//     {
//       image: vicerrectorImg,
//       name: 'M. Sc. Ing. David Soraide Lozano',
//       title: 'Vicerrector - Universidad Autónoma Tomás Frías',
//       onClick: () => navigate('/rutas-vicerrectorado'),
//     },
//   ];

//   return (
//     <div className="min-h-screen bg-[#F2F2F2] w-full">
//       {/* Línea roja superior */}
//       <div className="bg-[#AB2A2A] h-1 w-full" />

//       {/* Encabezado azul */}
//       <header className="bg-[#082F47] text-white py-4 text-center w-full">
//         <h2 className="italic text-xl sm:text-2xl">NUESTRAS AUTORIDADES</h2>
//       </header>

//       {/* Espaciador */}
//       <div className="my-6" />

//       {/* Contenedor de autoridades */}
//       <div className="flex flex-col md:flex-row gap-6 px-4">
//         {autoridades.map((persona, index) => (
//           <div key={index} className="w-full md:w-1/2">
//             {/* Imagen con clip y overlay */}
//             <div
//               onClick={persona.onClick}
//               className="relative group cursor-pointer w-full h-[300px] md:h-[400px] overflow-hidden rounded-xl shadow-md"
//               style={{
//                 clipPath:
//                   index === 0
//                     ? 'polygon(0 0, 100% 0, 85% 100%, 0% 100%)'
//                     : 'polygon(15% 0, 100% 0, 100% 100%, 0 100%)',
//               }}
//             >
//               <img
//                 src={persona.image}
//                 alt={persona.name}
//                 className="w-full h-full object-cover"
//                 loading="lazy"
//               />
//               {/* Overlay oscuro con mensaje */}
//               <div className="absolute inset-0 bg-black bg-opacity-50 text-white opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center text-xs font-semibold">
//                 Ingresa aquí...
//               </div>
//             </div>

//             {/* Texto inferior */}
//             <div className="p-4 md:p-6 bg-white text-black">
//               <h2 className="text-lg md:text-xl font-bold mb-1">{persona.name}</h2>
//               <p className="italic text-sm mb-2">{persona.title}</p>
//               {persona.paragraph && (
//                 <p className="text-sm text-justify whitespace-pre-line leading-relaxed">
//                   {persona.paragraph}
//                 </p>
//               )}
//             </div>
//           </div>
//         ))}
//       </div>

//       {/* Línea roja inferior */}
//       <div className="bg-[#AB2A2A] h-1 w-[95%] mx-auto my-6" />
//     </div>
//   );
// };

// export default BioSection;

// src/components/BioSection.jsx







import React, { useState, useEffect } from 'react';
import axios from 'axios';
import { useNavigate } from 'react-router-dom';
import { API_URL, BASE_URL } from '../../constantes/api';

const BioSection = () => {
  const [autoridades, setAutoridades] = useState([]);
  const [loading, setLoading] = useState(true);
  const navigate = useNavigate();

  useEffect(() => {
    const fetchAutoridades = async () => {
      try {
        const response = await axios.get(`${API_URL}autoridades`);

        // const admins = response.data.administrativos.map(a => ({
        //   id: a.id,
        //   nombre: a.administrativo.persona.nombres + ' ' + a.administrativo.persona.apellidos,
        //   cargo: a.cargo_regular.nombre,
        //   imagen: a.administrativo.persona.img,
        //   onClick: () => navigate('/rutas-rectorado'), // puedes personalizar por cargo
        // }));

        // const docentes = response.data.docentes.map(d => ({
        //   id: d.id,
        //   nombre: d.docente.persona.nombres + ' ' + d.docente.persona.apellidos,
        //   cargo: d.cargo_intermedio.nombre,
        //   imagen: d.docente.persona.img,
        //   onClick:
        //     d.cargo_intermedio.nombre.toLowerCase().includes('rector') 
        //       ? () => navigate('/rutas-rectorado')
        //       : () => navigate('/rutas-vicerrectorado'),
        // }));


       // 🔥 Solo tomamos Rector y Vicerrector
        const rectorVicerrector = response.data.docentes
          .filter(d =>
            ["rector", "vicerrector"].includes(d.cargo_intermedio.nombre.toLowerCase())
          )
          .map(d => ({
            id: d.id,
            nombre: `${d.docente.persona.nombres} ${d.docente.persona.apellidos}`,
            cargo: d.cargo_intermedio.nombre,
            imagen: d.docente.persona.img,
            onClick:
              d.cargo_intermedio.nombre.toLowerCase() === "rector"
                ? () => navigate("/rutas-rectorado")
                : () => navigate("/rutas-vicerrectorado"),
          }));

        setAutoridades(rectorVicerrector);
      } catch (error) {
        console.error("Error al obtener autoridades", error);
      } finally {
        setLoading(false);
      }
    };

    fetchAutoridades();
  }, [navigate]);
  
  if (loading) {
    return <p className="text-center text-gray-500">Cargando autoridades...</p>;
  }


  return (
    <div className="min-h-screen bg-[#F2F2F2] w-full">
      {/* Línea roja superior */}
      <div className="bg-[#AB2A2A] h-1 w-full" />

      {/* Encabezado */}
      <header className="bg-[#082F47] text-white py-4 text-center w-full">
        <h2 className="italic text-xl sm:text-2xl">NUESTRAS AUTORIDADES</h2>
      </header>

      {/* Contenedor */}
      <div className="flex flex-col md:flex-row gap-6 px-4 my-6">
        {autoridades.map((persona, index) => (
          <div key={persona.id} className="w-full md:w-1/2">
            {/* Imagen */}
            <div
              onClick={persona.onClick}
              className="relative group cursor-pointer w-full h-[300px] md:h-[400px] overflow-hidden rounded-xl shadow-md"
              style={{
                clipPath:
                  index === 0
                    ? "polygon(0 0, 100% 0, 85% 100%, 0% 100%)"
                    : "polygon(15% 0, 100% 0, 100% 100%, 0 100%)",
              }}
            >
              <img
                src={BASE_URL + persona.imagen || "/default-image.jpg"}
                alt={persona.nombre}
                className="w-full h-full object-cover"
                loading="lazy"
              />
              <div className="absolute inset-0 bg-black bg-opacity-50 text-white opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center text-xs font-semibold">
                Ingresa aquí...
              </div>
            </div>

            {/* Texto */}
            <div className="p-4 md:p-6 bg-white text-black">
              <h2 className="text-lg md:text-xl font-bold mb-1">{persona.nombre}</h2>
              <p className="italic text-sm mb-2">{persona.cargo}</p>
            </div>
          </div>
        ))}
      </div>

      {/* Línea roja inferior */}
      <div className="bg-[#AB2A2A] h-1 w-[95%] mx-auto my-6" />
    </div>
  );
};

export default BioSection;
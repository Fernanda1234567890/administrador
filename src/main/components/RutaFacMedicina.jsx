...
const RutaFacMedicina = () => {
  ...
  // Traer director desde backend
  useEffect(() => {
    const fetchDirector = async () => {
      try {
        // 👇 Cambia el id de carrera (ejemplo: Medicina = 1)
        const res = await axios.get('http://localhost:3000/api/carreras/1/director');
        setDirector(res.data.director);
      } catch (error) {
        console.error('Error al obtener director de Medicina', error);
      }
    };
    fetchDirector();
  }, []);
  ...
  return (
    <section className="min-h-screen bg-white flex items-center justify-center py-12 px-6 sm:px-20">
      <div className="text-center max-w-6xl mx-auto w-full">
        <h1 className="text-5xl font-bold text-center mb-8 text-blu-950 z-10">
          FACULTAD DE MEDICINA
        </h1>

        {/* Sección con Decano fijo */}
        <div className="mt-10 mb-10 flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
          <div className="text-center md:text-left md:w-1/2">
            <h2 className="text-2xl font-semibold text-blue-900">
              Dr. Jose Manuel Calle Aracena
            </h2>
            <p className="text font-semibold text-blue-900 mt-2">
              Decano de la Facultad de Medicina
            </p>
        ...
        <div className="flex justify-center gap-6 flex-wrap relative z-10">
          ...
          >
            ...
            {/* <img
        src={director.persona.img || '/carMed.jpg'} // usa la foto del director o imagen por defecto
        alt={`${director.persona.nombres} ${director.persona.apellidos}`}
        className="w-80 h-96 object-cover mx-auto mb-2 rounded-lg"
      /> */}
            <div className="content relative z-10 transform transition duration-300 translate-y-20 opacity-0 hover:translate-y-0 hover:opacity-100">
              <h1 className="text-xl font-semibold text-black mb-2">CARRERA MEDICINA</h1>
              <div className="mb-4">
                {director ? (
                  <>
                    ...
                    <h3 className="text-base font-medium text-gray-600 italic">
                      Director de Carrera
                    </h3>
                  </>
                ) : (
                  <p className="text-gray-400 italic">Cargando director...</p>
                )}
              </div>
            </div>
          </div>

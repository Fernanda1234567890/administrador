...
const DocenteVer = () => {
  ...
  const fetchDocentes = async () => {
    try {
      ...
      const res = await getData(page, limit, search, estado); // enviar estado correctamente
  ...
  return (
    <div className="p-6 sm:p-2 lg:p-12 min-h-screen dark:bg-white">
     ...
     <div className="mb-4 flex justify-between items-center">
      ...
      <button
          onClick={() => navigate("/admin/docentes/registrar")}
          ...
          <table className="w-full border-collapse bg-white shadow-lg rounded-lg">
            ...
            <tbody>
              {docentes.map((doc) => (
                ...
                {/* Carrera: acceder al nombre */}
                <td className="p-3">{doc.carrera?.nombre || "Sin carrera"}</td>
                
                {/* Persona: mostrar CI + nombres */}
                <td className="p-3">
                  {doc.persona ? `${doc.persona.ci} - ${doc.persona.nombres} ${doc.persona.apellidos}` : "Sin persona"}
                </td>                
                {/* Botones */}
                <td className="p-3 flex gap-2">
                  
                  <button
                    onClick={() => navigate(`/admin/docente/asignacion/${doc.id}`)}

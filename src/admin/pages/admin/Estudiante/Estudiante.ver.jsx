...
const EstudianteVer = () => {
  ...
  const fetchEstudiantes = async () => {
    try {
      ...
      const res = await estudiantesData().getData(page, limit, search, estado);
  ...
  const handleActualizar = async (est) => {
    ...
    try {
      await estudiantesData().update(est.id, { ru: parseInt(nuevoRu), carrera: nuevaCarrera });
      fetchEstudiantes();
    } catch (error) {
  ...
  const handleEliminar = async (id) => {
    ...
    try {
      await estudiantesData().remove(id);
      fetchEstudiantes();
    } catch (error) {
    ...
    const handleRestaurar = async (id) => {
      if (!window.confirm("¿Seguro que quieres restaurar este estudiante?")) return;

      try {
        await restore(id);
        alert("Estudiante restaurado correctamente");
        fetchEstudiantes();
      } catch (error) {
        console.error("Error al restaurar estudiante:", error);
        alert("No se pudo restaurar estudiante");
      }
    };


  return (
    <div className="p-6 sm:p-2 lg:p-12 min-h-screen bg-gray-50">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-2xl font-bold mb-4 text-gray-800">Estudiantes Registrados</h2> </div>
      ...
      <div className="mb-4 flex gap-2">
        ...
        />
        <button onClick={fetchEstudiantes} className="bg-blue-700 text-white px-3 py-1 rounded">
          Buscar
        </button>
      </div>
      <button
          onClick={() => navigate("/admin/estudiantes/registrar")}

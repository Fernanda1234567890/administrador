...
const tipoUnidadesData = () => {
  const getData = async (page = 1, limit = 10, search = "", estado = true) => {
    ...
    const getTiposParaSelect = async () => {
    try {
      const response = await axios.get(API_URL, {
        params: { page: 1, limit: 100, estado: 'activo' }
      });
      return response.data.data || [];
    } catch (error) {
      console.error("Error al obtener tipos para select:", error);
      return [];
    }
  };
 ...
 const restoreData = async (id) => {
    try {
      ...
      console.error("Error al restaurar tipo-unidad:", error);
      throw error;
    }
  };
